const { Mib, Import_mibs } = require('./mib_mib');
const { oid_list } = require('./mib_reader');
const { OidObject, OidObjectParam } = require('./mib_oid');
...
MibParser.prototype.parse_moduleIdentity = function () {
  ...
  while (true) {
    ...
    if (this.line.indexOf('LAST-UPDATED') > 0) {
      ...
      this.mib.origin = this.line
        .substring(this.line.indexOf('{') + 1, this.line.indexOf('}'))
        .trim();
...
MibParser.prototype.parse_object = function () {
  const oid_object = new OidObject();
  const oid_param = new OidObjectParam();
  ...
  while (true) {
    ...
    if (this.line.indexOf('SYNTAX') >= 0) {
      ...
      oid_param.setIndex(
        this.line.substring(this.line.indexOf('{') + 1, this.line.indexOf('}')).trim()
      );
    } else if (this.line.indexOf('DESCRIPTION') >= 0) {
      oid_param.setDescription(this.parse_description());
    } else if (this.line.indexOf('::=') >= 0) {
      oid_object.setOrigin(
        this.line.substring(this.line.indexOf('{') + 1, this.line.indexOf('}')).trim()
      );
      break;
    }
...
MibParser.prototype.parse_objectIdentifier = function (name) {
  ...
  if (line_split[0].trim() != 'OBJECT IDENTIFIER') {
    ...
    oid_object.mib = this.mib;
    oid_object.setOrigin(
      line_split[1].substring(line_split[1].indexOf('{') + 1, line_split[1].indexOf('}')).trim()
    );
    this.mib.oid_object.push(oid_object);
    oid_list.push(oid_object);
  }
